...
Разработчик WebProgrammer (kl.dm.vl@yandex.ru)
*/
...
Class SMSRU
	...
	*/
	public function sendSmtp($post)
		...
		if ($post->charset != $post->send_charset) $post->body = iconv($post->charset, $post->send_charset, $post->body);
		$headers = "From: $post->\r\n";
	...
	*/
	...
	На номера, добавленные в стоплист, не доставляются сообщения (и за них не списываются деньги)
		*$number = string - Номер телефона
		$text = string - Примечание (доступно только вам)
	*/
	...
	*/	
	...
	private function Request($url, $post=false)
		...
		if (isset($post->api_id)) { }
		else $post->api_id=$this->ApiKey;
